...
def main():
    ...
    more_cmds = ' '.join(sys.argv[2:])
    
    run_qemu(bios_patched_fp, floppy_img_fp, more_cmds)


def run_qemu(bios_fp, floppy_fp, more_cmds):
    cmd = f'qemu-system-i386 -drive format=raw,if=floppy,media=disk,file={floppy_fp} -bios {bios_fp} -net none -cpu base -monitor none -snapshot -no-reboot -sandbox on,obsolete=deny,elevateprivileges=deny,spawn=deny,resourcecontrol=deny -curses {more_cmds}'
    os.system(cmd)
